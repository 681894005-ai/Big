...
export const DB = {
  ...
  // --- EMPLOYEES (CRUD) ---
  getEmployees() {
    return JSON.parse(localStorage.getItem("restaurant_employees")) || DEFAULT_EMPLOYEES;
  },
  ...
  updateEmployee(username, updatedEmployee) {
    ...
    return { success: false, message: "ไม่พบพนักงานในระบบ" };
  },

  deleteEmployee(username) {
    if (username === "admin") {
      return { success: false, message: "ไม่สามารถลบผู้จัดการหลัก (admin) ได้" };
    }
    const employees = this.getEmployees();
    const filtered = employees.filter(e => e.username !== username);
